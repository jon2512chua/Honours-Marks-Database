...
public class BaseAssessment {
     ...
     */
    public BaseAssessment(String name, Unit parentUnit, int unitPercent) throws SQLException {
        try (Statement s1 = Session.dbConn.getConnection().createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
            s1.execute("INSERT INTO Assessment(AssessmentName, UnitCode, UnitPercent) VALUES ('" + name + "', '" + parentUnit.getUnitCode() + "', " + unitPercent + ")");
            
            // Assume that the following would just return 1 record.
            Statement s2 = Session.dbConn.getConnection().createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
            ResultSet assessmentRS = s2.executeQuery("SELECT AssessmentID FROM Assessment WHERE "
                    + "AssessmentName = '" + name + "' AND "
                    + "UnitCode = '" + parentUnit.getUnitCode() + "' AND "
                    + "UnitPercent = " + unitPercent);
            
            setAssessmentID(assessmentRS.getInt("AssessmentID"));
            setName(name);
